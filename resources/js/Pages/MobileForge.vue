...
import { DEFAULT_DEVICE_PRESET_ID } from './components/devicePresets.js';

const MANIFEST_KEY = 'mobileforge';
const VIEWPORT_KEY = 'mobileforge_viewport';

// --- PROJECT METADATA ---
const projectId = ref(props.projectKey);
const projectName = ref('Mobile Forge');
const database = ref([]);
...
const devicePresetId = ref(DEFAULT_DEVICE_PRESET_ID);
...
/** Full-bleed root container — no default text, empty children for drag-and-drop */
const generateMobileEmptyKit = (id) => [
  {
    id: `root_${id}`,
    type: 'container-v',
    props: {
...
const pages = ref([
  {
    id: 1, name: 'Home', slug: 'mobile-home', type: 'page',
    showPage: true, menuAction: 'None', children: [], elements: generateMobileEmptyKit(1)
  },
]);
...
function addPage(parent = null) {
  ...
  if(name) {
    ...
    const newPage = {
      ...
      showPage: true, menuAction: 'None', children: [], elements: generateMobileEmptyKit(id)
    };
...
const templateModules = import.meta.glob('./MobileTemplates/*.vue', { eager: true });
      ...
      data: {
        ...
        ui_state: {
          [VIEWPORT_KEY]: {
            ...
            y: panY.value,
            scale: scale.value,
            devicePresetId: devicePresetId.value,
          }
        }
      }
    };
    await axios.post(`/projects-mobileforge/${projectId.value}`, payload);
    alert('MobileForge Saved!');
...
const loadProject = async () => {
  try {
    ...
    if (data && data.data) {
      const manifest = data.data.manifest?.[MANIFEST_KEY] || {};
      const ui = data.data.ui_state?.[VIEWPORT_KEY] || {};
      pages.value = manifest.pages || pages.value;
      activePageId.value = manifest.activePageId || activePageId.value;
      const savedTemplate = manifest.selectedTemplate;
      const templateAliases = { BlueTopNav: 'BlueClean', LightSidebar: 'LightClean' };
      const resolved =
        (savedTemplate && templates.value[savedTemplate] && savedTemplate)
        || (savedTemplate && templateAliases[savedTemplate] && templates.value[templateAliases[savedTemplate]])
        || null;
      selectedTemplate.value = resolved ?? (templates.value.Empty ? 'Empty' : selectedTemplate.value);
      ...
      if (ui.devicePresetId) devicePresetId.value = ui.devicePresetId;
...
onMounted(async () => {
  ...
  if (!selectedTemplate.value || !templates.value[selectedTemplate.value]) {
    selectedTemplate.value = templates.value.Empty ? 'Empty' : Object.keys(templates.value)[0];
  }
  recordHistory();
  fetchActiveSession();
});
...
<template>
  <div class="flex w-full h-full bg-[#0d0f14] text-gray-300 overflow-hidden select-none font-sans">
    ...
    <ForgeCanvas
        ...
        v-model:devicePresetId="devicePresetId"
        saveLabel="Save Mobile"
